...
одним заходом не проверялся; проверен вариант Б (и В), а шагом `encode_catalog.py`
...
Проверено: независимый прогон варианта Б (и В) на чистой копии репозитория (от
предобработки до ответа) дал побайтово тот же `answer.csv`, что отправлен на
платформу.
