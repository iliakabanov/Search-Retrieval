...
одним заходом не проверялся; проверен вариант Б, а шагом `encode_catalog.py`
...
Проверено: независимый прогон варианта Б на чистой копии репозитория (от
предобработки до ответа) дал побайтово тот же `answer.csv`, что отправлен на
платформу; вариант со скачанными артефактами бенчмарка — тоже.
