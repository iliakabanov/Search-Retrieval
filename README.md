# Search-Retrieval — поиск объявлений с услугами
...
**Результат решения на бенчмарке: 87.75%.**
...
во всех командах — те, которыми получен отправленный файл.
...
То же, что вариант Б, но эмбеддинги считаются локально вместо скачивания:
ничего, кроме данных задания и весов открытых моделей, не нужно. Сквозной прогон
одним заходом не проверялся; проверены варианты Б и В, а шагом `encode_catalog.py`
посчитаны выложенные эмбеддинги.
...
### Б. Без кодирования эмбеддингов — примерно 1.5 часа (рекомендуется)
...
### В. Без отбора кандидатов и признаков — примерно 30 минут (рекомендуется)
...
**О точности воспроизведения.** Побайтово тот же `answer.csv`, что отправлен на
платформу, даёт сценарий В: он берёт ровно те признаки, на которых обучено
отправленное решение. Сценарии А и Б считают кандидатов и признаки сами, и ответ
может разойтись на доли процента: независимый прогон варианта Б на чистой копии
репозитория дал recall@50 94.66% на val-test против 94.52% у отправленного
решения. Разброс такого порядка на метрику не влияет (первый этап и потолок
кандидатов совпали точно), но `answer.csv` получается уже не побайтово тем же.
...
потолок кандидатов — 94.52% против 87.92% у первого этапа.
